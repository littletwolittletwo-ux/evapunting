...
import { CheckCircle2, Clock, Eye, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function Admin() {
  ...
  async function updateAccountStatus(accountId: string, status: 'pending' | 'connected') {
    try {
      ...
      if (selectedUser) {
        const updatedUserAccounts = selectedUser.accounts.map((acc) =>
          acc.id === accountId ? { ...acc, status } : acc
        );
        setSelectedUser({ ...selectedUser, accounts: updatedUserAccounts });
      }
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (filter === 'awaiting')
      return user.accounts.some((acc) => acc.status === 'pending');
  ...
  if (loading) {
    return (
      <div className="bg-gray-50 dark:bg-gray-900 flex items-center justify-center h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 dark:border-blue-600"></div>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-300 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 lg:mb-8 gap-4">
          <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent">Admin Panel</h1>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-3 py-2 sm:px-4 text-sm sm:text-base rounded-lg font-medium transition-all duration-200 ${
                filter === 'active'
                  ? 'bg-gradient-to-r from-green-600 to-blue-600 text-white shadow-lg shadow-green-500/50 dark:shadow-green-600/50'
                  : 'bg-gray-200 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-800 border border-gray-300 dark:border-gray-700/50'
              }`}
            >
              Active
            ...
            <table className="w-full min-w-[640px]">
              <thead className="bg-gray-200 dark:bg-gray-800/50 border-b border-gray-300 dark:border-b-gray-700/50">
                <tr>
                  ...
                  <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">
                    Email
                  </th>
                  <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">
                    Accounts
              ...
              <tbody className="divide-y divide-gray-300 dark:divide-y-gray-700/50">
                {filteredUsers.map((user) => {
                  const awaitingCount = user.accounts.filter(
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-gray-200 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400">{user.accounts.length}</td>
                      <td className="px-3 sm:px-6 py-3 sm:py-4">
      ...
      {showModal && selectedUser && (
        <div className="fixed inset-0 bg-gray-900/50 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-300 dark:border-gray-700/50 rounded-xl max-w-2xl w-full p-4 sm:p-6 my-8 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <h3 className="text-lg font-semibold text-blue-600 dark:text-blue-400 mb-3">Connected Bookmaker Accounts</h3>
            ...
            {selectedUser.accounts.length === 0 ? (
              ...
              <div className="space-y-4">
                {selectedUser.accounts.map((account) => (
                  <div key={account.id} className="bg-gray-200/50 dark:bg-gray-900/30 border border-gray-300 dark:border-gray-700/50 rounded-lg p-4 hover:bg-gray-300/50 dark:hover:bg-gray-900/50 transition-colors">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1">
                        <h4 className="font-semibold text-gray-900 dark:text-gray-100 capitalize">{account.bookmaker_name}</h4>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Username: {account.username}</p>
                        {account.email && (
                          <p className="text-sm text-gray-600 dark:text-gray-400">Email: {account.email}</p>
                        )}
                        {account.password && (
                          <p className="text-sm text-gray-600 dark:text-gray-400">Password: {account.password}</p>
                        )}
                        {account.deposit_amount && (
                          <p className="text-sm text-gray-600 dark:text-gray-400">Deposit: ${account.deposit_amount}</p>
                        )}
                      </div>
                      <span
                      ...
                      >
                        {account.status === 'connected' ? 'Connected' : account.status === 'error' ? 'Error' : 'Pending'}
                      </span>
                    </div>
